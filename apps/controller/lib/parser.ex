defmodule Parser do
  ...
  def compare_intake([first | [second | [{"CASTLE"}]]], placements, turn_color) do
    ...
    case moving_placement do
      ...
      {other_color, piecetype} -> {:error, "wrong color at indicated start location"}
    end
  end
    ...
    case inferPawnStartLocation(only) do
      ...
      start ->
        ...
        case moving_placement do
          ...
          {other_color, piecetype} ->
            {:error, "wrong color at indicated start location"}
        end
    end
  end

  def compare_intake(any), do: {:error, "cool not valid at compare intake step"}
  ...
  def parse(:col, rest, acc), do: Enum.reverse(acc)
